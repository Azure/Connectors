...
//   node packages/connector-namespaces/preview/server.mjs
...
async function main() {
    if (!hasAgentBrowser()) {
        ...
        console.log("Then re-run:      node packages/connector-namespaces/preview/shots.mjs");
        process.exit(0);
    }

    if (!(await serverUp())) {
        console.error("preview server is not reachable at " + BASE);
        console.error("start it first:  node packages/connector-namespaces/preview/server.mjs");
        process.exit(1);
    }
