...
const activityFunction: AzureFunction = async function (context: Context): Promise<EarliestEvents> {
    ...
    const calendarResponse = await outlook.calendarsTable.calendarGetTables();
    const calendar = calendarResponse.value.filter((tableElement) => {
        return tableElement.displayName == "Calendar"
    })[0];
    ...
    const eventResponse = await outlook.calendarsTableData.getEventsCalendarViewV3(calendar.name, startTime, endTime);
